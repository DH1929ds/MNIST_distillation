...
class DDPM(nn.Module):
    ...
    def _sample(self, n_sample, size, device, guide_w=0.0, class_label=None):
        """
        Conditional sampling 함수
        - n_sample: 샘플 개수
        - size: 생성할 이미지의 크기 (채널, 높이, 너비)
        - device: 사용할 장치 (cuda 또는 cpu)
        - guide_w: guidance weight 값
        - class_label: 샘플링할 클래스 라벨
        """
        x_i = torch.randn(n_sample, *size).to(device)  # x_T ~ N(0, 1), 초기 노이즈 샘플링
        ...
        # 테스트 시 context_mask를 사용하지 않음 (모든 컨텍스트 사용)
        context_mask = torch.zeros_like(c_i).to(device)
    
        # 배치 더블링
        c_i = c_i.repeat(2)
        context_mask = context_mask.repeat(2)
        context_mask[n_sample:] = 1.  # 배치의 두 번째 절반에 대해 context-free 설정
    
        x_i_store = []  # 샘플링 진행 과정을 저장할 리스트
