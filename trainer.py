...
class distillation_DDPM_trainer(nn.Module):
    def __init__(self, T_model, S_model, distill_features = False, cfg_update = False):
    ...
    def forward(self, xt, c, t, feature_loss_weight = 0.1):
        """
        Perform the forward pass for knowledge distillation.
        """
        ############################### TODO ###########################
        
        context_mask = torch.where(c == 10, torch.ones_like(c), torch.zeros_like(c)).to(xt.device)
        c = torch.where(c == 10, torch.zeros_like(c), c)
        
        self.T_model.eval()
        
        if self.cfg_update:
            # guide_w = random.choice([0, 0.5, 2.0])
            guide_w = 0.5
        else:
            guide_w = 0.0
        ...
        if self.distill_features:
            ...
            with torch.no_grad():
                #teacher_output, teacher_features = self.T_model.forward_features(x_t, t)
                x_prev, T_output, T_features = self.T_model.cache_step(xt,c,t,context_mask, guide_w)
                

            #student_output, student_features = self.S_model.forward_features(x_t, t)
            self.S_model.train()
            _, S_output, S_features = self.S_model.cache_step(xt,c,t,context_mask)
